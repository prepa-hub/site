...
class UserController extends Controller
{
    ...
    public function update(Request $request, $id)
    {
        ...
        if (!$request->ajax()) {
            $oldStatus = User::findOrfail($id)->status;
            $newStatus = $request->status == 'on' ? 1 : 0;
            $this->insertUpdateData($request, $slug, $dataType->editRows, $data);
            // Notify user on active !
    ...
    public function uploadImage(Request $request)
    {
        ...
        Auth::user()->save();
        return back()->with('success', 'Photo de profile mise à jour !');
    }
    ...
    public function updateProfile(Request $request)
    {
        ...
        if ($request->input('subject_id')) {
            Auth::user()->subject_id = $request->input('subject_id');
        }
        if ($request->input('password')) {
            Auth::user()->password = Hash::make($request->input('password'));
            Auth::user()->save();
            return back()->with('success', 'Mot de passe mise à jour !');
        }
        Auth::user()->save();
        return back()->with('success', 'Profile mise à jour !');
    }
